...
def update(frame):
    ...
    if i%countPeriod==0:    #执行测距
        ...
        datatemp={'time step':0,'distance':0}
        datatemp['time step']=timeStep
        timeStep=timeStep+1
        datatemp['distance']=calcuDis([x_u[0],y_u[0]],[x[0],y[0]])
        log_data=log_data.append(datatemp,ignore_index=True)
